#!/usr/bin/env bash
#
# 拼 multi-arch manifest list 推到指定 registry。从 DIGESTS_DIR 收 build matrix
...
#   TAGS         换行分隔的 tag 列表(直接传 docker/metadata-action 的 outputs.tags)
# 可选 env:
#   DIGESTS_DIR  digest 文件目录,默认 /tmp/digests
#
# 必需文件:
#   ${DIGESTS_DIR}/<arch>  build matrix 各 arch 写入的 digest(单行 sha256:...)
...
if [ "${#digest_files[@]}" -eq 0 ]; then
	echo "::error::digest directory is empty: $digest_dir"
	exit 1
fi
