...
**这是本插件在 Koishi 4 上的最后一个版本。** 本插件在 Koishi 4 上到此暂停,等 Koishi 升到 v5 再跟进更新。近期会推出一个薄的 Koishi 适配插件,把 bilibili-notify 独立端(Docker / 桌面版)桥接进 Koishi,之后的新功能都在独立端上做。这一版只做一件事:把 5.2.0 装不上的 Node 20 用户救回来。

### Fixed

- **Node 20 重新可用**:5.2.0 把 Node 要求抬到了 22.22,但 Koishi Desktop 自带的 Node 是 20.12.2、用户换不了,装上插件后一加载就报 `ERR_REQUIRE_ESM`(渲染用的 jsdom 30 那条依赖链在 Node 20.19 / 22.12 之前的版本上跑不起来)。现在把 jsdom 退回不受影响的 26 系列,Node 要求放回 `>=20`;**5.2.0 那条「Node 20 上装不上」的说明作废**。推送、渲染、指令、配置项均无变化 (2484810f)

---

## [5.2.0] — 2026-08-31

一版依赖维护:底层依赖整体升到当前主线,插件功能与配置一字未改。**但 Node 版本要求提高了 —— Node 20 上装不上,升级前请先看下面第一条。**

### Changed

- ⚠️ **Node 版本要求提高到 `^22.22.2 || ^24.15.0 || >=26`**:渲染用的 jsdom 升到 30,它自己要求这个下限。**还在 Node 20 上的话,升级插件前得先升 Node**,否则装不上 —— 此前 `>=20` 的承诺就此作废 (2c497488)
- **底层依赖整体升级**:jsdom 30、定时任务库 cron 4、AI SDK(openai)7,以及一批 patch/minor 更新。三者都是内部实现,**推送行为、指令、配置项均无变化**,升级即可 (55696606, df05ae16, 2c497488, b006202f, 8c80a4a0)

---

## [5.1.2] — 2026-08-28

直播信息流客户端换了底:第三方库退役,换成自己实现的 WSS 客户端 —— 推送行为不变,换来的是风控后的自动恢复和连接稳定性;并配上协议漂移告警,B 站悄悄改字段不再静默漏消息。
...
纯修复版,两个方向:AI 请求的抗抖动(限流、慢开场、卡流、超时),和一个能把整个插件卡死的正则漏洞。

### Fixed

- **AI 请求抗抖动一轮**(动态点评 / 直播总结 / `bili chat` 都受益):撞到限流(429)按网关给的 `Retry-After` 时刻回来重试,超过 20 秒的直接报错不空等;流式回复给首块更长的宽限 —— 思考型模型(如 DeepSeek 开思考)开场要想很久,以前会被当成没响应误杀;中途卡死的流会被看门狗掐掉重试;超时不再降级成非流式把慢活白干一遍 (d59626d, 4f2a96a, e4bb101, 867a351, a1f10c1)
- **正则安全闸门放过了有界重复的嵌套构造**:高级订阅的黑白名单正则里写下 `(a{0,30}){0,30}` 这类模式会被保存放行,动态检测一撞上匹配就把整个 koishi 进程卡死。现在运行时的安全闸会拒绝执行这类正则并在日志里说明 (0483929)

---

## [5.1.0] — 2026-08-14

AI 侧三件新事:联网搜索回归(这次是女仆自己的搜索工具,谁家模型都能用)、新增阿里云百炼、接上 OpenAI 的 Responses 协议;另修掉「DeepSeek 开思考一调工具就 400」和「下播了又推正在直播」两个毛病。

### Added

- **联网搜索回归**:AI 配置新增「联网搜索」—— 搜索由**博查**或 **Tavily** 真正执行(`web_search` 工具),跟选哪家 AI 服务商无关,DeepSeek 官方也能联网。(5.0.0-alpha.12 移除的旧开关是各家 LLM 自带的联网方言,分裂且 DeepSeek 压根没有;这次是女仆自己的工具,谁家都能用。)动态点评 / 直播总结 / `bili chat` 三条路各自独立开关,**全部默认关** —— 搜索按次计费,得主人亲手点亮。两家 key 各存一格,换后端不丢 (01df44c, 63cb0a5, aaf4557, 28605a4)
- **阿里云百炼(DashScope)服务商**:服务商列表新增百炼,思考开关 / 深度按它家 `enable_thinking` 的写法自动翻译 (5395655)
- **接口风味(Responses API)**:OpenRouter / 百炼 / DeepSeek / 自定义分支下新增「接口风味」—— `chat`(经典 `/chat/completions`,默认、行为不变)或 `responses`(OpenAI 2025 起的接任协议;DeepSeek 已生产级支持、百炼 qwen3-max 系可用、OpenRouter 还在 beta)。「自定义」档选 responses 后思考开关会亮出来:那套协议里思考是标准字段,不需要方言翻译,OpenAI 官方正是这么接的 (024e9a1, d7f2e39, 0077cd9)

### Fixed

- **DeepSeek 开思考时一调工具就 400**:DeepSeek v4 要求把思考内容原样回传给工具调用的后续轮次,流式路径把这个字段丢了 —— 群聊里「边想边查」(查订阅 / 看图)的回合必炸 (648487d)
...
两条都在动态这一侧:检测出故障和恢复正常时会私聊说一声,以及动态里带的关联视频会渲染成视频卡片。

### Added

- **动态检测出故障时会直接私聊主人**:账号被风控(会提示用 `bili cap` 解除)、或拉取动态报错时,除了原有的告警,现在还会私聊说一声;**恢复正常之后也会私聊报个平安**,不必自己去翻日志确认它到底缓过来没有 (a4927ba)
...
两条女仆的提示词修复。都是「她说要去做点什么,然后就没有然后了」那一类 —— 一个在点评上空转,一个在群聊里声称帮你改好了订阅。

### Fixed

- **动态点评和直播总结会空转**:回一句「我得先确认一下我这边有没有订阅这个 UP 主…让我先看看订阅情况,稍等一下哦!」然后整条点评就这么没了。点评这条路一个工具都没挂,提示词却在要求她「查订阅必须调工具」,她只好用自然语言演一遍 (5aa1973)
- **群聊女仆会声称已经帮你订阅 / 退订,其实什么也没发生**。那些工具在 5.0.0-alpha.12 就整体移除了(那条路径没有权限校验),提示词却一直没跟上,还在指挥她去调用并不存在的工具。现在她会直说自己做不到,并请你去面板或指令里操作 (4c9e747)

---

## [5.0.0] — 2026-07-31

5.x 正式版 —— npm 的 `latest` 从 4.2.0 挪到这里,以后不加 `@alpha` 装到的就是 5.x。相对上一个 alpha,这版都在出图上:多图动态跟着 B 站官方改成九宫格、GIF 标出「动图」,并修掉两个一直都在的老毛病 —— 含 GIF 的动态推送会凭空少几张图,以及卡片字体设置**从来就没生效过**。

⚠️ **从 4.2.0 直接升上来的主人请先看这段。** 5.x 是一次大改,4.2.0 之后的变更全散在下面 `5.0.0-alpha.0` ~ `5.0.0-alpha.12` 各段里,其中三条会让你动手:

- **五个插件合并成了一个**:`-dynamic` / `-live` / `-ai` / `-advanced-subscription` 不再需要,请卸载它们、只留 `koishi-plugin-bilibili-notify`(详见 `5.0.0-alpha.8`)
- **配置结构整个按功能域重排**成 `account` / `push` / `subscriptions` / `render` / `ai` / `dynamic` / `live` / `advancedSub` 八段,升级后要对照控制台重新填一遍(详见 `5.0.0-alpha.8`)
- **群聊女仆的订阅管理工具已整体移除**,「联网搜索」开关不再存在(详见 `5.0.0-alpha.12`)
...
- **卡片字体设置从来没生效过**,填什么字体名都不作数。⚠️ 现在生效了,也就是说**升级后卡片字体可能真的变了** (6922d59)
