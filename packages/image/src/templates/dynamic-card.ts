...
export type DynamicContent = {
	/** 主体内容 HTML */
	html: string;
	/** 转发时的标签（如 "投稿了视频"），仅在 isForward=true 时可能出现 */
	forwardLabel?: string;
	/** 追加到 pubTime 的后缀（如 " · 投稿了视频"），仅在 isForward=false 时可能出现 */
 ...
 * 根据动态类型构建内容 HTML
 * @param dynamic 动态数据
 * @param isForward 是否作为被转发动态（影响标签文字的去向）
...
export async function buildDynamicContent(
	...
	switch (dynamic.type) {
		...
		case DYNAMIC_TYPE_DRAW: {
			const html = buildBasicContent(dynamic, false, dirname);
			const additionalHtml = buildAdditionalContent(dynamic);
			return { html: html + additionalHtml };
		}
		...
		case DYNAMIC_TYPE_FORWARD: {
			...
			if (!dynamic.orig)
				return {
					html: `${selfContent + additionalHtml}<p>${upName}转发了一条动态，但原动态已不可见</p>`,
				};
			// 递归构建被转发动态
			...
			const forwardBlock = buildForwardBlock(
				forwardedAuthor.face,
				forwardedAuthor.name,
				forwarded.forwardLabel,
				forwarded.html,
			);
			return { html: selfContent + additionalHtml + forwardBlock };
		}

		case DYNAMIC_TYPE_AV: {
			const selfContent = buildBasicContent(dynamic, false, dirname);
			// 判断是否有视频内容
			if (!dynamic.modules.module_dynamic?.major?.archive) return { html: selfContent };
			const archive = dynamic.modules.module_dynamic.major.archive;
			const isNewVideo = archive.badge.text === "投稿视频";
			const videoHtml = buildVideoContent(archive);
			return {
				html: selfContent + videoHtml,
		...
		case DYNAMIC_TYPE_ARTICLE: {
			const html = buildBasicContent(dynamic, true, dirname);
			return {
				html,
		...
		case DYNAMIC_TYPE_LIVE:
			return {
				html: `<p>${upName}发起了直播预约，我暂时无法渲染，请自行查看</p>`,
			};
		case DYNAMIC_TYPE_MEDIALIST:
			return {
				html: `<p>${upName}分享了收藏夹，我暂时无法渲染，请自行查看</p>`,
			};
		case DYNAMIC_TYPE_PGC:
			return {
				html: `<p>${upName}发布了剧集（番剧、电影、纪录片），我暂时无法渲染，请自行查看</p>`,
			};
		case DYNAMIC_TYPE_MUSIC:
			return { html: `<p>${upName}发行了新歌，我暂时无法渲染，请自行查看</p>` };
		case DYNAMIC_TYPE_COMMON_SQUARE:
			return {
				html: `<p>${upName}发布了装扮｜剧集｜点评｜普通分享，我暂时无法渲染，请自行查看</p>`,
			};
		case DYNAMIC_TYPE_COURSES_SEASON:
			return {
				html: `<p>${upName}发布了新课程，我暂时无法渲染，请自行查看</p>`,
			};
		case DYNAMIC_TYPE_UGC_SEASON:
			return { html: `<p>${upName}更新了合集，我暂时无法渲染，请自行查看</p>` };
		case DYNAMIC_TYPE_NONE:
			return { html: `<p>${upName}发布了一条无效动态</p>` };
		case DYNAMIC_TYPE_LIVE_RCMD:
			throw new Error("直播开播动态，不做处理");
		default:
			return { html: `<p>${upName}发布了一条我无法识别的动态，请自行查看</p>` };
	}
}
...
/** 处理图文、专栏等基础内容 */
function buildBasicContent(dynamic: Dynamic, isArticle: boolean, dirname: string): string {
	const mod = dynamic.modules.module_dynamic;
	let html = "";

	if (mod?.desc?.rich_text_nodes) {
		html += parseRichText(mod.desc.rich_text_nodes, undefined, isArticle);
	}
	if (mod?.major?.opus?.summary?.rich_text_nodes) {
		html += parseRichText(mod.major.opus.summary.rich_text_nodes, mod.major.opus.title, isArticle);
	}
	if (mod?.major?.opus?.pics) {
		html += buildPicsContent(mod.major.opus.pics, dirname);
	}
	return html;
}
...
function buildPicsContent(
	...
	const arrowImg = pathToFileURL(resolve(dirname, "img/arrow.png"));
			...
			return `
                <div class="card-major">
                    <div class="single-photo-container">
                        <img class="single-photo-item" src="${pic.url}"/>
	...
	const photoClass = pics.length === 4 ? "four-photo-item" : "photo-item";
	const photosHtml = pics.map((p) => `<img class="${photoClass}" src="${p.url}"/>`).join("");
	return `<div class="card-major">${photosHtml}</div>`;
}
	...
	return `
        <div class="card-forward">
            <div class="forward-userinfo">
                <img class="forward-avatar" src="${avatarUrl}" alt="avatar">
                <span class="forward-username">${username}${label}</span>
            </div>
...
/** 构建附加卡片（预约、商品等） */
function buildAdditionalContent(dynamic: Dynamic): string {
...
function buildReserveAdditional(reserve: any): string {
	const isEnded = reserve.button.uncheck.text === "已结束";
	const button = isEnded
		? `<button class="reserve-button-end"><span>${reserve.button.uncheck.text}</span></button>`
	...
	return `
        <div class="card-reserve">
            <div class="reserve-main">
                <div class="reserve-title">${reserve.title}</div>
            ...
            <div class="reserve-button">${button}</div>
        </div>
    `;
}

/** 商品卡片 */
const SHOPPING_ICON_SVG = `<svg style="width: 12px; height: 12px;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 12" width="12" height="12">
...
function buildGoodsAdditional(goods: any): string {
	const headText = `<div class="up-recommand">${SHOPPING_ICON_SVG} ${goods.head_text}</div>`;
                ...
                <div class="reserve-main">
                    <div class="reserve-desc" style="display: flex; gap: 10px; align-items: center;">
                        ...
                        <div style="flex: 1; min-width: 0;">
                            ...
                            <div style="font-size: 16px; color: #FF6699; font-weight: bold;">${item.price}</div>
                        </div>
                    </div>
                </div>
	...
	return `
        ...
        <div class="card-reserve">
            <div class="reserve-main">
                <div class="reserve-desc">${itemsHtml}</div>
            </div>
        </div>
    `;
}
                        ...
                        <div class="card-body">
                            ...
                            <div class="card-content">
                                <div class="card-header">
                                    <div class="up-info">
                                        <div class="up-name" style="${p.upIsVip ? "color: #FB7299" : ""}">${p.upName}</div>
                                        <div class="pub-time">${p.pubTime}</div>
                                    </div>
                                    ${
																			p.decorateCardUrl
																				? `
                                    <div class="dress-up">
                                        <img src="${p.decorateCardUrl}" />
                                        <span>${p.decorateCardId}</span>
                                    </div>
                                    `
																				: ""
																		}
                                </div>
                                <div class="card-topic">${topicHtml}</div>
                                ${p.mainContent}
                                <div class="card-stat">
                                    <div class="stat-item">
                                        <svg style="width: 18px; height: 18px;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18" width="18" height="18">
                                            <path d="M9.789075 2.2956175C8.97235 1.6308450000000003 7.74999 2.212005 7.74999 3.26506L7.74999 5.3915500000000005C6.642015000000001 5.5780325 5.3073725 6.040405 4.141735000000001 7.11143C2.809155 8.335825 1.751515 10.3041 1.45716 13.404099999999998C1.409905 13.9018 1.7595399999999999 14.22505 2.105415 14.317499999999999C2.442215 14.40755 2.8807175 14.314625 3.127745 13.92915C3.9664525 12.620249999999999 4.89282 11.894575 5.765827499999999 11.50585C6.4628049999999995 11.19545 7.14528 11.093125 7.74999 11.0959L7.74999 13.235025C7.74999 14.2881 8.97235 14.869250000000001 9.789075 14.2045L15.556199999999999 9.510425000000001C16.355075 8.860149999999999 16.355075 7.640124999999999 15.556199999999999 6.989840000000001L9.789075 2.2956175z" fill="currentColor"></path>
                                        </svg>
                                        <span>${p.forwardCount}</span>
                                    </div>
                                    <div class="stat-item">
                                        <svg style="width: 18px; height: 18px;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18" width="18" height="18">
                                            <path d="M1.5625 7.875C1.5625 4.595807499999999 4.220807499999999 1.9375 7.5 1.9375L10.5 1.9375C13.779175 1.9375 16.4375 4.595807499999999 16.4375 7.875C16.4375 11.0504 13.944675 13.6435 10.809275 13.80405C10.097025 14.722974999999998 8.920875 15.880675 7.267095 16.331325C6.9735075 16.4113 6.704762499999999 16.286224999999998 6.55411 16.092325C6.40789 15.904149999999998 6.3561 15.634350000000001 6.4652449999999995 15.383025C6.72879 14.776249999999997 6.776465 14.221025000000001 6.7340175 13.761800000000001C3.8167675 13.387125 1.5625 10.894475 1.5625 7.875z" fill="currentColor"></path>
                                        </svg>
                                        <span>${p.commentCount}</span>
                                    </div>
                                    <div class="stat-item">
                                        <svg style="width: 18px; height: 18px;" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18" width="18" height="18">
                                            <path d="M10.4511 2.2220125C10.218425 2.194885 10.002175 2.2953725 9.884175 2.433395C9.4264 2.9688525 9.321875 3.7501399999999996 8.978575 4.3581725C8.533574999999999 5.146395 8.1198 5.6213375 7.609775000000001 6.068507499999999C7.1751375 6.449565 6.738407499999999 6.697442499999999 6.3125 6.8050575L6.3125 14.854575C6.9198900000000005 14.868174999999999 7.572900000000001 14.876875 8.25 14.876875C9.936425 14.876875 11.367025 14.823325 12.33115 14.773699999999998C13.03235 14.737575 13.646025000000002 14.390075 13.966750000000001 13.81945C14.401900000000001 13.04535 14.9387 11.909650000000001 15.264174999999998 10.571200000000001C15.56665 9.327275 15.704699999999999 8.304325 15.766675 7.582224999999999C15.7988 7.208262500000001 15.50165 6.875019999999999 15.059999999999999 6.875019999999999L11.323274999999999 6.875019999999999C11.156575 6.875019999999999 11.000800000000002 6.791952499999999 10.907975 6.653499999999999C10.783725 6.468192500000001 10.82855 6.2670175 10.9037 6.07485C11.059 5.675084999999999 11.29355 4.9974475 11.382425000000001 4.4018275C11.470875000000001 3.80917 11.450999999999999 3.32219 11.212050000000001 2.86913C10.9571 2.3857825 10.66065 2.2464475 10.4511 2.2220125z" fill="currentColor"></path>
                                        </svg>
