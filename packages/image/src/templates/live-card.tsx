...
export type LiveCardInput = {
	...
	description?: string;
	/**
	 * **生效的**封面(远端网址或 data URL)—— 用哪一张由来源自己定(B 站:直播中先关键帧、
	 * 其余先房间封面,缺了拿另一张兜)。主人给这条订阅设的自定义直播封面在出卡时另行覆盖在
	 * 它上面。不给(或空串)且没有自定义封面时,卡上画 BN 自带的占位图。
	 */
	cover?: string;
	/**
...
export type LiveCardView = {
	...
	description: string;
	/**
	 * 生效的封面(自定义直播封面已经覆盖上去了),**永远画得出来**:没有真封面时是 BN 自带的
	 * 占位图(`LIVE_COVER_PLACEHOLDER`),不是空串 —— 空的 `src` 画出来是一张裂图。
	 */
	cover: string;
	/** 有没有**真的**封面(自定义封面或来源给的那张);占位图不算。 */
	hasCover: boolean;
	/** 「开播时间：…」/「直播时长：…」/「未开播」那一句。 */
	time: string;
	online: string;
