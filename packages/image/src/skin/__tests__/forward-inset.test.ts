...
function innerLabels(doc: Document): string[] {
	const inset = doc.querySelector('[data-bn="forward"]');
...
describe("转发框里的内层卡 — 跟着皮肤走", () => {
	it("块的顺序按皮肤,不是出厂默认版式", async () => {
		...
		const doc = await render([block("c", "content", 1), block("h", "header", 2)]);
		expect(innerLabels(doc)).toEqual(["content", "header"]);
	});

	it("皮肤没摆的块,内层也不画", async () => {
		const doc = await render([block("c", "content", 1)]);
		expect(innerLabels(doc)).toEqual(["content"]);
	});

	it("内层块挂的是同一批 bn-blk- class —— 一条皮肤 CSS 同时管内外两层", async () => {
		const doc = await render([block("h", "header", 1), block("c", "content", 2)]);
...
describe("转发框里的内层卡 — 数据各算各的", () => {
	...
	it("showIf 按内层的数据判 —— 外层是转发、内层不是", async () => {
		const doc = await render([block("c", "content", 1), customBlock("tag", "<div>转发</div>", 2)]);
		...
		expect(innerLabels(guarded)).toEqual(["content"]);
	});
});
