 ...
 *
 * 守护契约:
 *   - 有 passphrase → PassphraseKeyProvider(protected=true);
 *     getKey 确定、缓存;salt 文件落盘;同 salt+passphrase 跨实例一致;换 passphrase 变
 *   - 无 passphrase → FileKeyProvider(protected=false);
 *     getKey 生成 master.key 并缓存
 */
...
describe("createKeyProvider — 选型", () => {
	it("有 passphrase → Passphrase(protected)", () => {
	...
	it("空白 passphrase 视为未提供 → File(回退)", () => {
...
describe("FileKeyProvider", () => {
	it("getKey 生成 master.key 并缓存", async () => {
		const { keyPath } = paths();
