...
describe("searchImages — 找壁纸用的图片候选", () => {
	it("博查:图片在 data.images.value,取 contentUrl 与尺寸", async () => {
		fetchMock.mockResolvedValueOnce(
			jsonRes({
				code: 200,
				data: {
					...
					images: {
						value: [
							{
								name: "雷姆 壁纸",
								contentUrl: "https://img.example.com/rem.jpg",
								hostPageUrl: "https://example.com/p/1",
								width: 1920,
								height: 1080,
							},
							// contentUrl 缺失的整条丢掉 —— 下不下来的候选摆出来只会白挑一次
							{ name: "没图", width: 100, height: 100 },
						],
					},
				},
			}),
		...
		const ex = createWebSearchExecutor({ backend: "bocha", apiKey: "sk-test" });
		const images = await ex.searchImages("雷姆 壁纸");

		expect(images).toEqual([
			{
				title: "雷姆 壁纸",
				url: "https://img.example.com/rem.jpg",
				width: 1920,
				height: 1080,
	...
	it("Tavily:要显式请求图片,结果在 images", async () => {
		fetchMock.mockResolvedValueOnce(
			jsonRes({
				results: [],
				images: [
					{ url: "https://img.example.com/a.png", description: "蓝发女仆" },
					"https://img.example.com/b.png",
				],
			}),
		);
		const ex = createWebSearchExecutor({ backend: "tavily", apiKey: "tvly-x" });
		const images = await ex.searchImages("rem wallpaper");
		...
		expect(images[0]?.title).toBe("蓝发女仆");
	});

	it("非 http(s) 的候选直接丢 —— data: 之类的东西不该进下载器", async () => {
		fetchMock.mockResolvedValueOnce(
			jsonRes({
				code: 200,
				data: {
					images: {
						value: [
							...
							{ contentUrl: "https://img.example.com/ok.png" },
						],
					},
				},
			}),
		...
		expect((await ex.searchImages("x")).map((i) => i.url)).toEqual([
