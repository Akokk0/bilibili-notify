...
| `OptionCard` | 一张可选的卡:左边一枚 `KindMark`(28,`logo` 或 `mark` 的头两个字母)+ 右边加粗的 `label`,两张一排摆(声明式 `enum` 选项带图标时,桥的「哪一种桥」就是)。选中整句吃 `SELECTED_LANGUAGE`、方块换 16% 粉底粉字、`label` 转正文色;未选中中性描边,悬停描边加深一档(`text-tertiary`,不转粉)。带 `aria-pressed`,挂 `option` / `option-active`。⚠️ 备份弹窗的 `ChoiceCard`(web `pages/backup/dialog-bits.tsx`)是同一种东西、**还没合并**:那件没有方块、是「标题 + 小字」两行,悬停转粉,没有 `aria-pressed` —— 合并前别再照着任何一件另抄第三份 |
