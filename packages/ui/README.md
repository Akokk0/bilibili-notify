...
| `MonoChip` | 等宽小字的浅灰底胶囊(`<span>`,只收字符串):地址、token、遮住的密钥这种「一串要照抄的东西」装在它里面。宽度 / 截断 / 内边距走 `className`(遮住的密钥那一格是 `min-w-0 flex-1 truncate`) |
...
| `TriStateMark` / `TriStateChip` / `TRISTATE_TEXT` / `TriState` | 三态记号:支持 / 不支持 / 还不知道(拓展视图 tristate 列那种)。三档靠**形状**分 —— 实心打勾 / 空心圈里一横 / 虚线空圈,颜色只是第二条通道;🔴 **「不支持」与「还不知道」不许并成一档**(前者是结论,后者是「试试看」),也**不许拿删除线画「不支持」**(删除线在这套界面里读作「坏了」,主人真这么读错过一张正常的能力表)。`TriStateMark` 是记号本身,**图例与正文共用这一颗**;`TriStateChip` = 记号 + 名字,悬停说明用 `TRISTATE_TEXT` 那三句。图例(`TriStateLegend`)住 web 的 `pages/extensions/declarative/parts.tsx` |
...
| `QrPanel` | 扫码那一块:224px 见方的二维码(白底留边 + 浮起)+ 底下的说明(`children`,一个子节点一行)。系统页的 B 站登录、拓展的 `qr` 积木、QQ 扫码绑定共用 —— **任何扫码都该长一个样**,收编前是三份复制粘贴。`src` 没有时给了 `loading` 就占一个与图同尺寸的方块、里面转圈说那句话,不给就不占位。`variant`:`framed`(默认)自带描边浅底的卡;`bare` 除了图本身什么底和边都不画,给**已经在弹窗里**的位置(卡里套卡、占位再垫底都多余)。外层属性透传(导览锚点 `data-tour` 挂这儿)。web 的 `library-reuse-conformance` 拦手摆的那一方 |
...
| `KindMark` | 「哪一种」那枚方块:中性灰 12% 底 + 同色字,里面是 `logo`(图片 data URL,走 `<img>`)或 `text` 的头两个字母。三档 `size` 是三个位置:26 表格 icon 格 / 28 选项卡片 / 32 列表卡左上。**走中性灰不走语义色** —— 种类不是状态,给了语义色就跟卡上真正的状态抢注意力;选中要换色的(选项卡片)走 `style` 整个盖掉。给 `label` 是一枚有名字的图(`role="img"`),不给是旁边那行字的装饰(`aria-hidden`)。与 `PlatformIcon` 的分工:那个查注入的平台表,这个不查表,只画拿到的 logo 或字母 |
| `OptionCard` | 一张可选的卡(不是按钮):带 `aria-pressed`,挂 `option` / `option-active`,选中整句吃 `SELECTED_LANGUAGE`。**方块与说明各自可选**,用到的是两种形态 —— **有方块**(给 `mark`,可带 `logo`):左边一枚 `KindMark`(28)+ 右边加粗的 `label`,两张一排摆(声明式 `enum` 选项带图标时,桥的「哪一种桥」就是);**没方块**:加粗标题 + `description` 一行小字(备份弹窗的完整/脱敏、覆盖/合并)。🔴 **选中的粉只落一处**:有方块 → 方块换 16% 粉底粉字、`label` 转正文色;没方块 → 标题自己转粉 —— 两处都粉就是一张卡把「选中」说了两遍。字号跟行数走:带 `description` 的两行卡标题 `base`,单行 `sm`。未选中中性描边,悬停描边转淡粉(`bn-pink/40`,两形态统一)。两形态的内边距与未选中字色是合并前各自的原样,别顺手拉平。备份那件原是 web 里的 `ChoiceCard`,已并进来 —— 再要「可选的卡」就用它,别另抄 |
...
| `GlassBox` | 重玻璃分区卡(Rules/Cards/AI 页那种):icon 芯片 + badge + 右侧动作槽 + 分隔线。`mark` 是**自绘**的图标位(不套 accent 芯片 —— 桥接入卡的角光跟连接状态走、方块却印「哪一种桥」,两者不该同色);`aside` 摆在 badge 之后同一行,给不是 accent 色的东西(状态点 + 「已连接」) |
