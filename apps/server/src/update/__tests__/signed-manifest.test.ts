...
describe("loadSignedManifest", () => {
	it("备用公钥签的 manifest 照样验得过 —— 双密钥保险的命门", () => {
		...
		const bytes = Buffer.from(JSON.stringify({ version: "0.9.0" }), "utf8");
	...
	it("签名验过了但不是一份清单的形状 → 拒绝,不把垃圾当 Manifest 交出去", () => {
		...
		const notManifests = [
			{ notVersion: "0.9.0" }, // 缺 version
			{ version: 900 }, // version 不是字符串
			[{ version: "0.9.0" }], // 顶层是数组
			null, // 合法 JSON,但什么都不是
		];
	...
	it("内容被改过一个字节 → 拒绝", () => {
		const key = makeKey();
		const original = Buffer.from(JSON.stringify({ version: "0.9.0" }), "utf8");
		const signature = signBytes(key.privateKey, original);

		// 代理站是**设计上的中间人** —— 它终止 TLS、把字节重新发给我们。签名是我们
		// 唯一能确认「这坨字节没被那个中间人动过」的手段。
		const tampered = Buffer.from(JSON.stringify({ version: "9.9.9" }), "utf8");
