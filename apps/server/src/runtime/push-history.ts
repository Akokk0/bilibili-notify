 ...
 *
 * 推送层只知道订阅 id / feature / 目标 / 消息与结果;历史行还要 B 站 uid、推送类型、以及 UP
...
export function historyRecordFromSend(
	...
	// 拓展订阅没有 uid,而 `HistoryEntry.uid` 今天还是必填。拓展订阅的历史行形状归 ④「接进
	// 推送链」那一片定(ADR-0019 决策 50);这一片里只有 B 站适配器调 broadcastToFeature,走不到这儿。
	if (!isBiliSubscription(subscription)) return null;
	return {
		pushId: info.pushId,
		kind: info.kind,
