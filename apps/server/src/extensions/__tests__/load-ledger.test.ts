...
function attempt(id: string, version = "1.0.0") {
	return recordLoadAttempt({ root, id, version, maxFailures: MAX });
}
...
describe("拓展失败记账", () => {
	it("第一次要加载 → 记一笔,还没到上限", () => {
		expect(attempt("bridge")).toEqual({ attempts: 1, blocked: false });
	});

	it("连着到上限 → 自动停用,而且是**这一次**就停(判死记在选中那一刻)", () => {
		attempt("bridge");
		attempt("bridge");
		expect(attempt("bridge")).toEqual({ attempts: MAX, blocked: true });
	...
	it("换一版就重新给机会 —— 记账键带版本,不必去哪里手动解封", () => {
		...
		attempt("bridge");
		attempt("bridge");
		expect(attempt("bridge", "1.0.1")).toEqual({ attempts: 1, blocked: false });
		// 老那版还关着:修好的是新版,不是它。
		expect(readLoadLedger(root).blocked).toEqual(["bridge@1.0.0"]);
	});
	...
	it("一个拓展记账不牵连另一个", () => {
		...
		attempt("bridge");
		attempt("bridge");
		expect(attempt("douyin")).toEqual({ attempts: 1, blocked: false });
	});
	...
	it("落盘的是一份看得懂的 JSON —— 主人要能自己打开删一行", async () => {
		attempt("bridge");
		const raw = JSON.parse(await readFile(join(root, "load-state.json"), "utf8"));
		expect(raw).toEqual({ attempts: { "bridge@1.0.0": 1 }, blocked: [] });
	});
});
