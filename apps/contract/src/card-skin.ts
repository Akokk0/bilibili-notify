...
export interface CardSkinPreviewResponse {
	...
	scene: string;
	/**
	 * 这一场落在哪个**形态**(`null` = 只有基础版式)。形态是真机分得出来的那几种卡,
	 * 皮肤按它存覆盖版式(ADR-0014 决策 10 的 2026-09-18 🔗)。
	 *
	 * 由服务端回报而不是面板照场景 id 猜:两张预览图本来就可能落在同一档(动态卡的
	 * 「全字段」与「视频投稿」都带视频卡),各算一遍的话,面板会把改动写进一份根本没人
	 * 用的覆盖里,而两边的门禁全绿。
	 */
	variant: string | null;
}
