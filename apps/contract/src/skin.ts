...
export const SKIN_CSS_HOOK_NOTES: Record<SkinCssHook, string> = {
	...
	modal: "弹窗卡片本体",
	badge:
		"不可点的小徽章 —— 顶栏「服务器运行中」那类状态胶囊,以及行内那种紧凑计数/标签徽章(「订阅 UP 主 3」的那个 3)。**非交互**,别写 hover;底色多是语义色(平台色、推送类型色,不少还是行内样式),盖 background 会让徽章说谎,通常只描边加影。**同一挂点两种形态**:顶栏那种有内边距撑着,行内那种靠行高定高、上下没有余量 —— 描边一律用 outline+负 offset,写 border 会把它撑高 2px 顶开整行",
	"nav-item":
	...
	"option-active":
		"选中的那一行额外挂它(与 option 同挂);选中态写这儿,别写在 option 上让整列全亮。注意有些候选的选中色是**行内样式**的品牌色(服务商卡、适配器行那种),盖 background 盖不动,描边加影最稳",
