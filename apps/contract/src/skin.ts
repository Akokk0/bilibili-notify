...
export const SKIN_CSS_HOOK_NOTES: Record<SkinCssHook, string> = {
	...
	modal: "弹窗卡片本体",
	badge:
		"不可点的小徽章 —— 顶栏「服务器运行中」那类状态胶囊,以及行内那种紧凑计数/标签徽章(「订阅 UP 主 3」的那个 3)。**非交互**,别写 hover;底色多是语义色(平台色、推送类型色,不少还是行内样式),盖 background 会让徽章说谎,通常只描边加影。**同一挂点两种形态**:顶栏那种有内边距撑着,行内那种靠行高定高、上下没有余量 —— 描边一律用 outline+负 offset,写 border 会把它撑高 2px 顶开整行。**描边颜色写 `var(--bn-tint)`**:每颗徽章都把自己那个语义色露在这个变量里,写死一个颜色会把「直播是粉的、动态是蓝的」整个罩掉,一排徽章长得一模一样",
	"nav-item":
