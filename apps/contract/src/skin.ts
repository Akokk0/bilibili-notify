 ...
 *
 * 放在契约里,是因为当初聊天里的内置设计师与粘给外部 AI 的那份提示词**都要教这件事**,
 * 只教一边的话另一边会把同一个坑再踩一次(后者已删,见 {@link SKIN_BEST_PRACTICES})。
 */
export const SKIN_CSS_HOOK_NOTES: Record<SkinCssHook, string> = {
	...
	"option-active":
		"选中的那一行额外挂它(与 option 同挂);选中态写这儿,别写在 option 上让整列全亮。有几列候选的选中底是**品牌/平台色**染的(服务商卡、连接行),盖 background 盖得动 —— 但盖掉就等于把「这是哪一家」这条信息抹了,想留着就只描边加影",
...
/**
 * 官方皮肤库的统一手感,原样拼进 AI 提示词。
 *
 * 这几条是**与受众无关的硬事实**(哪些字段该配什么值),不是语气 —— 而它曾在
 * 服务端与 web 各存一份措辞不同的副本。git 记录里 77201cc / acdf5a5 / 2af2191
 * 每次都同时改两边,付过三次双份账;真正的代价是漏改一边之后,两条造皮肤的
 * 路会教出两套不同规格的 AI,而构建全绿,只有真机上看得出来。下面这一组提示词
 * 常量都是因此搬进契约的。
 *
 * web 那份(复制给外部 AI 的提示词)已随 ADR-0015 决策 27 删掉,现在只剩服务端
 * `buildSkinAiSystemPrompt` 一处在拼(一键改写与皮肤工坊共用)。常量仍留在契约,
 * 不为一次删除再搬一回家。
 */
...
/**
 * 皮肤 CSS 的**属性白名单**(视觉层)。清洗层照这份放行,提示词照这份讲。
 *
 * 从前它只住在服务端的清洗层里,提示词那句「属性走视觉白名单:background/border/…
 * 等」是手抄的一小半,靠一个「等」字兜住剩下的 —— 于是 transform-origin、rotate、
 * top/left、content 这些**收得进去的**属性,当时两条造皮肤的路都没教过 AI。数据搬
 * 上来之后,加一个属性只改这一处,提示词跟着变。
 */
...
/**
 * CSS 属性白名单在提示词里的说法 —— 与 {@link SKIN_BEST_PRACTICES} 同一个理由:
 * 服务端与 web 的两份提示词曾各存一份,措辞已经漂开(一份列了 outline/text-shadow,
 * 另一份没有)。漏改一边不会红,只会让两条造皮肤的路教出两套规格的 AI —— 白名单
 * 加一个属性时尤其明显,那正是当初收口的由头。
 *
 ...
 * 一个大椭圆),不是审美偏好 —— 所以它属于「硬事实」那一类,每条造皮肤的路都得教。
 */
 ...
 * 于是从「粘贴到任意 AI」那条路(已删)造出来的皮肤理直气壮地给宿主写 position,
 * 而清洗层一句不留地剔掉它(顶栏靠 sticky 吸顶,被顶掉就散架 —— 那正是这道闸的由来)。
 * 「伪元素三件事不用操心」那段当时更是只有服务端有,web 那条路的 AI 一直在浪费声明。
 *
