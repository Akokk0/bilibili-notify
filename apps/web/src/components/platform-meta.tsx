 ...
 *
 * 🔴 **拓展那一档不在注册表里,也不许在这儿手抄** —— 它的短名与标识色是拓展自己报的
 * (`ExtensionDTO.push.display`:v2 写在清单里,v1 在 `activate` 里注册推送源时报),经
 * `/api/ext` 下来。
 * 这里一度躺着一行手抄的桥接粉紫,它的毛病不是丑,是**会跟拓展报的悄悄漂开**:改了
 * 拓展、面板还是旧色,而类型、测试、门禁全绿,只有真机上眼睛看得出来。
 */
 ...
 * 取值正是拓展 id。
 *
 * **只认有没有外观,不认在不在跑**(ADR-0019 决策 41):v2 的外观服务端照清单给、停着也有,
 * 拓展停了,它借来的连接照样画这张脸 —— 在这儿补一道「在跑才进表」,就又退回灰方章了。
 * 没交外观的(停着的 v1,外观写在代码里、问不出)**不进表**:退成灰方章是诚实的,而塞一份
 * 空壳进去等于面板自己编了一张脸。
 */
export function buildPlatformTable(
	...
	for (const ext of extensions) {
		const display = ext.push?.display;
		// 图标是清单里那段 SVG(服务端过过白名单,决策 20)—— 没有就退首字方章。
		if (display) table[ext.id] = { tint: display.color, label: display.shortLabel, svg: ext.icon };
	}
