	...
	return (
		<div data-testid={testId} className="flex flex-col gap-1.5">
			<button
				...
				aria-expanded={open}
				onClick={() => setOpen((v) => !v)}
				className="bn-glass flex w-fit cursor-pointer items-center gap-1.5 rounded-bn-card px-2.25 py-0.75 text-[11.5px] font-semibold text-bn-text-tertiary transition-colors hover:bg-[var(--bn-glass-strong-bg)] hover:text-bn-text-secondary"
			>
				{icon}
				{label}
	...
	return (
		<div className="flex flex-wrap gap-1.5">
			{traces.map((t, i) => {
				...
				const chipClass = `bn-glass hover:bg-[var(--bn-glass-strong-bg)] flex max-w-full items-center gap-1.5 rounded-bn-card px-2.25 py-0.75 text-[11.5px] text-bn-text-tertiary${clipped ? " cursor-pointer transition-colors hover:text-bn-text-secondary" : ""}`;
