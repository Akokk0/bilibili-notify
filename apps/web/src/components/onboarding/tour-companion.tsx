 ...
 *   「按过测试」才算配好连接,老用户升级后一律被判成没配完,不问一句就开导览
...
export function TourCompanion() {
	...
	// - 达成(doneWhen):探测数据证明子步目标已完成(如连接已落库),立刻翻页把灯
	//   移到下一个动作上(真机踩过:保存连接后灯断档,要自己想起来去点测试)。
	...
	// (reconcileTourPos),但子步曾只进不退 —— 删掉连接后 activeKey 仍是
