...
/** 这道闸看得见的那半:别的块占着哪儿,以及它**是不是一定会出现**。 */
...
/**
 * 同一片格子上**最多叠两层**(2026-09-18 主人:「三重及以上也删了,场景太少,表达很难」;
 * 推翻 09-16 那次的「三重都还好」)。
 *
 * 这个数正是**画布画得清的层数**:错位只有一档 —— 压着别人的往右下让开 8 / 4px,被压住的
 * 停在原地。两层刚好是「一条露边 + 一块浮着」;三层起,上面两块会重合在同一档上,读不出
 * 谁压着谁。
 *
 * ⚠️ **这是画布拖拽的闸,不是数据契约。** 检查器的行列数字框是精确编辑(ADR 说过它不该
 * 有副作用)、皮肤包 schema 也照旧放行 —— 手写的、别人分享的皮肤装得进来,装进来之后
 * 画布上会是挤的,但出图那头叠几层一点问题都没有(层次照样表达得清)。三处分别拦不拦由
 * 主人 2026-09-16 拍板。
 */
export const MAX_SOLID_STACK = 2;
 ...
 * 「现在谁跟谁叠着」(画布照它画深浅),这儿问的是「落过去会不会叠成第三层」。两份算法
