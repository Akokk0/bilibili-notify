这一章覆盖导览的第二、三、四步:接好推送连接 → 指定目标 → 测试打通。还没想好走哪条路?先看[总览的选型表](/about/guide)。
...
1. 打开[推送目标](/targets)页 → 添加连接 → 平台选「QQ 官方机器人」。
...
2. 「开发 → 开发设置」里拿 `AppID` 与 `AppSecret`,填进 BN 的连接表单。
...
1. 连接测试通过后,添加推送目标:群聊填 `groupOpenid`、私聊填 `userOpenid`。
...
OneBot 的连接方式有三种,**推荐反向 WS**(BN 开端口等 NapCat 连进来,NapCat 重启自动重连,最省心):

1. BN 侧:添加连接 → 平台选 OneBot → 连接方式选「反向 WS」→ 填一个监听端口(如 `6199`),保存。
...
## 路线 C:群机器人 / HTTP 端点(不用 QQ)

在连接的平台里挑「**飞书机器人**」「**钉钉机器人**」「**企业微信机器人**」其中一档,填目标 URL 即可;自己的服务走「**未指明的 HTTP 端点**」(BN 原生 envelope,随便接)。带 secret 的平台把签名密钥填上;端点架在反向代理或 Cloudflare Access 后面的,凭据填进「自定义请求头」那一栏。同样点「测试」验证。

## 通道验收

回到[推送目标](/targets)页,在目标行点「测试」—— QQ(或你配的那个端)收到测试消息,通道就全线打通了。最后一步:去[订阅](/about/guide/subs)想关注的 UP(记得勾上刚建好的推送目标),订阅上即毕业 🎉
