 ...
 * - **盖掉了一份这个进程跑过的** —— 那份代码已经 import 过,ESM 在这个进程里干净地换不掉
 *   (ADR-0012 决策 10),新版等着换上:重启 BN,或只重载这个拓展(决策 47);
 * - **包不合规** —— 服务端那几句是主人手里那个包哪里不对的唯一线索,逐条摆出来。
 *
 * ⛔ **那颗重启按钮只在确实需要它的这件事旁边出现**(ADR-0005 决策 22):这里,与详情页上
 * 那个拓展有新版等着换上时。面板上没有常驻的重启入口 —— 重启是**刚做完这件事**的后果,
 * 不是一个随时可按的动作。这台机器上按了回不来(开发版 / 裸跑)就不给按钮,但要把原因
 * 写出来。
 */
