...
**token 不放在 URL、也不放在帧里**：URL 会进反向代理的访问日志。token 不对 / 已吊销时
upgrade 直接回 **HTTP 401**，WebSocket 根本不会建起来 —— 你的客户端会收到
`unexpected-response`（Node 的 `ws`）或握手异常（Python 的 `websockets`），
请把它当成**配置错误**显示给用户，别无限重连。
