...
- **在分期 ①② 拆那两个千行文件**:见决策 15。
- **写版本文件保证回退安全**:旧构建根本不读它(同 ADR-0005)。⚠️ 别误读成「版本号也不要」——`CONFIG_SCHEMA_VERSION` 定义着(`packages/internal/src/schema/migration.ts`)、也在驱动**形状探测**与迁移日志,不落盘的只是那个版本**文件**。降格落地后复评过一次,结论不变:兜底是迁移留下的两份 `.bak`。
