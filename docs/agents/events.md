...
| `state` | `hydrate`(订阅 / 重连时)+ `config-changed`(只带 scope)+ `extension-changed` / `extension-settings-changed`(都只带拓展 id) | `useStateChannel` → 按 scope / 按拓展 id invalidate tanstack-query 缓存 |
